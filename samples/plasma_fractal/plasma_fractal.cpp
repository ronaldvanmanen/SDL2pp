...
vector<argb8888> generate_palette();
...
int main()
{
    ...
    while (running)
    {
        ...
        {
            texture.with_lock(
                [palette, plasma_fractal_image, palette_start](sdl2::image<argb8888> &pixels)
                {
                    for (int y = 0; y < pixels.height(); ++y)
                    {
                        for (int x = 0; x < pixels.width(); ++x)
                        {
                            size_t palette_offset = palette_start + plasma_fractal_image(x, y);
                            if (palette_offset >= palette.size())
                            {
                                palette_offset -= palette.size();
                            }
                            pixels(x, y) = palette[palette_offset];
                        }
                    }
                });
            ...
            if (palette_start >= palette.size())
            {
                palette_start -= palette.size();
            }
        }
    }
